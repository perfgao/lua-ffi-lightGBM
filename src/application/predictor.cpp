...
Predictor::Predictor(Boosting* boosting, int num_iteration,bool is_raw_score,
        bool predict_leaf_index, bool predict_contrib, bool early_stop,
        int early_stop_freq, double early_stop_margin) {
    early_stop_ = CreatePredictionEarlyStopInstance("none",
    ...
    if (early_stop && !boosting->NeedAccuratePrediction()) {
        ...
        if (boosting->NumberOfClasses() == 1) {
            early_stop_ = CreatePredictionEarlyStopInstance("binary",
    ...
    boosting_ = boosting;
    num_pred_one_row_ = boosting_->NumPredictOneRow(num_iteration,
            predict_leaf_index, predict_contrib);
    num_feature_ = boosting_->MaxFeatureIdx() + 1;
    predict_buf_ = std::vector<std::vector<double>>(num_threads_,
    ...
    if (predict_leaf_index) {
        ...
        if (is_raw_score) {
            predict_fun_ = [=](const std::vector<std::pair<int,
                    double>>& features, double* output) {
                int tid = omp_get_thread_num();
                if (num_feature_ > kFeatureThreshold &&
                        features.size() < KSparseThreshold) {
                    auto buf = CopyToPredictMap(features);
...
void Predictor::Predict(const char* data_filename, const char* result_filename,
        bool header) {
    auto writer = VirtualFileWriter::Make(result_filename);
    if (!writer->Init()) {
        Log::Fatal("Prediction results file %s cannot be found",
                result_filename);
    }

    auto parser = std::unique_ptr<Parser>(Parser::CreateParser(data_filename,
                header, boosting_->MaxFeatureIdx() + 1, boosting_->LabelIdx()));

    if (parser == nullptr) {
        Log::Fatal("Could not recognize the data format of data file %s",
                data_filename);
    }
...
void Predictor::Predict(std::vector<std::pair<int, double>>& oneline_features,
        double *result)
{
     predict_fun_(oneline_features, result);
}


void Predictor::CopyToPredictBuffer(double* pred_buf,
        const std::vector<std::pair<int, double>>& features) {
...
void Predictor::ClearPredictBuffer(double* pred_buf, size_t buf_size,
        const std::vector<std::pair<int, double>>& features) {
...
std::unordered_map<int, double> Predictor::CopyToPredictMap(
        const std::vector<std::pair<int, double>>& features) {
